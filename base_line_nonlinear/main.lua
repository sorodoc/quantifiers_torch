...
local function train(images, images_q)
    ...
    for n = 1, N do
        local freq_vector = torch.DoubleTensor(g_params.batchsize, #g_vocab - 3)
        local one_hot_vector = torch.DoubleTensor(g_params.batchsize, #g_vocab - 3)
        freq_vector:fill(0.0)
        one_hot_vector:fill(0.0)
        for b = 1, g_params.batchsize do
            local start = (n - 1) * g_params.batchsize
            --print(start+b)
            ...
            for i = 1, 16 do --change the parameter
              ...
              local x = g_vectors[images[start + b][i]]
              local count_sym = torch.dot(x, context[b][i])
              freq_vector[b][images[start + b][i] - 3] = freq_vector[b][images[start + b][i] - 3] + count_sym 
            end
            --local transpose_context = context[b]:transpose(1,2):contiguous()
            --local v_context = transpose_context:view(g_params.vector_size, g_params.memsize)
            --local v_input = input[b]:view(1, g_params.vector_size)
            --local sim_vector = torch.mm(v_input, v_context)
            --for i = 1, 16 do
            --  freq_vector[b][images[start + b][i] - 3] = freq_vector[b][images[start + b][i] - 3] + sim_vector[1][i]
            --end
            --print(freq_vector[b])
            one_hot_vector[b][images_q[start + b][1] -3] = 1.0
        end
        
        local x = {freq_vector, one_hot_vector, target}
...
cmd:option('--vocab_size', 16, 'size of the vocab of the symbols')
